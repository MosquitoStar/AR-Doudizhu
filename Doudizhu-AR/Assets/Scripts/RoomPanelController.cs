...
public class RoomPanelController : PunBehaviour {
	...
	void OnEnable () {
		...
		for (int i = 0; i < teamSize; i++) {	
			if (!Team1 [i].activeSelf) {		//在队伍1找到空余位置
				...
				PhotonNetwork.player.SetCustomProperties (costomProperties);	//将玩家自定义属性赋予玩家
				break;
			} else if (!Team3 [i].activeSelf) {	//在队伍2找到空余位置
				Team3 [i].SetActive (true);		//激活对应的队伍信息UI
				texts = Team3 [i].GetComponentsInChildren<Text> ();		//显示玩家昵称
				if(PhotonNetwork.isMasterClient)texts[1].text="房主";	//如果玩家是MasterClient，玩家状态显示"房主"
				else texts [1].text = "未准备";							//如果玩家不是MasterClient，玩家状态显示"未准备"
				costomProperties = new ExitGames.Client.Photon.Hashtable () {	//初始化玩家自定义属性
					{ "Team","Team3" },		//玩家队伍
	...
	void UpdateTeamPanel(bool isUpdateSelf){
		...
		foreach (PhotonPlayer p in PhotonNetwork.playerList) {	//获取房间里所有玩家信息
			...
			if(p.isMasterClient){							//如果玩家是MasterClient
				...
				if(costomProperties["Position1"].Equals("AI")){
					go = Team1[(int)costomProperties ["TeamNum"]];
					go.SetActive (true);
				}
				if(costomProperties["Position2"].Equals("AI")){
					go = Team2[(int)costomProperties ["TeamNum"]];
					go.SetActive (true);
				}
				if(costomProperties["Position3"].Equals("AI")){
					go = Team3[(int)costomProperties ["TeamNum"]];
					go.SetActive (true);
				}
			}
		...
		if (costomProperties ["Team"].ToString ().Equals ("Team1")) {				//判断玩家队伍
			for (int i = 0; i < teamSize; i++) {									//寻找另一支队伍是否有空余位置
				if (!Team2 [i].activeSelf) {										//如果找到了空缺位置
					...
					{ { "Team","Team2" }, { "TeamNum",i }, { "Type", "Man"}  };
					PhotonNetwork.player.SetCustomProperties (costomProperties);	
					break;
				}else if (!Team3 [i].activeSelf) {										//如果找到了空缺位置
					isSwitched = true;												//标记玩家切换队伍成功
					Team1 [(int)costomProperties ["TeamNum"]].SetActive (false);	//禁用之前显示玩家信息的UI
					texts = Team3 [i].GetComponentsInChildren<Text> ();				//获取切换队伍后，显示玩家信息的Text组件
					texts [0].text = PhotonNetwork.playerName;						//填入玩家昵称
					if(PhotonNetwork.isMasterClient)texts[1].text="房主";			//如果玩家是MasterClient，玩家状态显示"房主"
					else texts [1].text = "未准备";									//如果玩家不是MasterClient，玩家状态显示"未准备"
					Team3 [i].SetActive (true);										//激活显示玩家信息的UI
					costomProperties = new ExitGames.Client.Photon.Hashtable ()		//重新设置玩家的自定义属性
					{ { "Team","Team3" }, { "TeamNum",i }, { "Type", "Man"}  };
					PhotonNetwork.player.SetCustomProperties (costomProperties);	
					break;
				}
			}
			...
			for (int i = 0; i < teamSize; i++) {									//寻找另一支队伍是否有空余位置
				if (!Team3 [i].activeSelf) {										//如果找到了空缺位置
					isSwitched = true;												//标记玩家切换队伍成功
					...
					PhotonNetwork.player.SetCustomProperties (costomProperties);
					break;
				} else if (!Team1 [i].activeSelf) {										//如果找到了空缺位置
					isSwitched = true;												//标记玩家切换队伍成功
					Team2 [(int)(costomProperties ["TeamNum"])].SetActive (false);	//禁用之前显示玩家信息的UI
					...
					costomProperties = new ExitGames.Client.Photon.Hashtable ()		//重新设置玩家的自定义属性
					{ { "Team","Team1" }, { "TeamNum",i }, { "Type", "Man"}  };
					PhotonNetwork.player.SetCustomProperties (costomProperties);
					break;
				}
			}
		} else if (costomProperties ["Team"].ToString ().Equals ("Team3")) {		//判断玩家队伍
			for (int i = 0; i < teamSize; i++) {									//寻找另一支队伍是否有空余位置
				if (!Team1 [i].activeSelf) {										//如果找到了空缺位置
					isSwitched = true;												//标记玩家切换队伍成功
					Team3 [(int)(costomProperties ["TeamNum"])].SetActive (false);	//禁用之前显示玩家信息的UI
					texts = Team1 [i].GetComponentsInChildren<Text> ();				//获取切换队伍后，显示玩家信息的Text组件
					texts [0].text = PhotonNetwork.playerName;						//填入玩家昵称
					if(PhotonNetwork.isMasterClient)texts[1].text="房主";			//如果玩家是MasterClient，玩家状态显示"房主"
					else texts [1].text = "未准备";									//如果玩家不是MasterClient，玩家状态显示"未准备"
					...
					PhotonNetwork.player.SetCustomProperties (costomProperties);
					break;
				} else if (!Team2 [i].activeSelf) {										//如果找到了空缺位置
					isSwitched = true;												//标记玩家切换队伍成功
					Team3 [(int)(costomProperties ["TeamNum"])].SetActive (false);	//禁用之前显示玩家信息的UI
					texts = Team2 [i].GetComponentsInChildren<Text> ();				//获取切换队伍后，显示玩家信息的Text组件
					texts [0].text = PhotonNetwork.playerName;						//填入玩家昵称
	...
	public void ClickStartGameButton(){
		...
		pView.RPC ("LoadGameScene", PhotonTargets.All, "Coloring3D");	//调用RPC，让游戏房间内所有玩家加载场景GameScene，开始游戏
	}
	...
	//添加电脑按钮响应函数
	public void AddAIButtonControl(){
		if(PhotonNetwork.isMasterClient){
			Debug.Log("AddAIButtonControl");
			addAIButton.gameObject.SetActive(true);
			addAIButton.onClick.RemoveAllListeners ();						//移除ReadyButton所有监听事件
			addAIButton.onClick.AddListener (delegate() {					//为ReadyButton绑定新的监听事件
				ClickAddAIButton ();		
			});
		}
		else {
	...
	public void ClickAddAIButton(){
		Debug.Log("ClickAIButton");
		bool isAdded = false;	
		for (int i = 0; i < teamSize; i++) {		
			if (!Team1 [i].activeSelf) {										//如果找到了空缺位置
				isAdded = true;												//标记玩家切换队伍成功
				texts = Team1 [i].GetComponentsInChildren<Text> ();				//获取切换队伍后，显示玩家信息的Text组件
				texts [0].text = "电脑";					
				texts [1].text = "已准备";	
				Team1 [i].SetActive (true);										//激活显示玩家信息的UI
		...
		if (!isAdded)
			promptMessage.text = "房间人数已满,无法添加电脑";	//如果玩家切换队伍失败，提示信息显示"另一队伍已满，无法切换"
		else
			promptMessage.text = "";
	}
	//删除电脑按钮响应函数
	public void DeleteAIButtonControl(){
		if(PhotonNetwork.isMasterClient){
			deleteAIButton.gameObject.SetActive(true);
			deleteAIButton.onClick.RemoveAllListeners ();						//移除ReadyButton所有监听事件
			deleteAIButton.onClick.AddListener (delegate() {					//为ReadyButton绑定新的监听事件
				ClickDeleteAIButton ();				
			});
		}
		else {
			deleteAIButton.gameObject.SetActive(false);
		}
	}
	public void ClickDeleteAIButton(){
		bool isDeleted = false;			
		if (PhotonNetwork.player.customProperties["Position3"].Equals("AI")) {										//如果找到了空缺位置
			isDeleted = true;												//标记玩家切换队伍成功
			Team3 [(int)costomProperties ["TeamNum"]].SetActive (false);	//禁用之前显示玩家信息的UI
			costomProperties = new ExitGames.Client.Photon.Hashtable ()		//重新设置玩家的自定义属性
			{ {"Position3", "Man"} };
			PhotonNetwork.player.SetCustomProperties (costomProperties);
		}else if (PhotonNetwork.player.customProperties["Position2"].Equals("AI")) {										//如果找到了空缺位置
			isDeleted = true;												//标记玩家切换队伍成功
			Team2 [(int)costomProperties ["TeamNum"]].SetActive (false);	//禁用之前显示玩家信息的UI	
			costomProperties = new ExitGames.Client.Photon.Hashtable ()		//重新设置玩家的自定义属性
			{ {"Position2", "Man"} };
			PhotonNetwork.player.SetCustomProperties (costomProperties);
		}else if (PhotonNetwork.player.customProperties["Position1"].Equals("AI")) {										//如果找到了空缺位置
			isDeleted = true;												//标记玩家切换队伍成功
			Team1 [(int)costomProperties ["TeamNum"]].SetActive (false);	//禁用之前显示玩家信息的UI
			costomProperties = new ExitGames.Client.Photon.Hashtable ()		//重新设置玩家的自定义属性
			{ {"Position1", "Man"} };
		...
		if (!isDeleted)
			promptMessage.text = "没有电脑玩家，无法删除";	//如果玩家切换队伍失败，提示信息显示"另一队伍已满，无法切换"
		else
			promptMessage.text = "";
	}
}
